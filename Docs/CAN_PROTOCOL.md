...
当前仅实现 P04 `thr_hit` 的 RAM 写入。参数写入 ID 为每个 NodeID 的业务基址加 `0x0B`，DLC=8：Byte0=`0x04`，Byte1~4 为阈值 `uint32` 小端，Byte5=0（不保存），Byte6 为事务序号，Byte7=0。合法范围为 `1000~67108864`，成功后立即更新运行中的击打判定阈值。ACK 使用业务基址加 `0x0C`，DLC=8：Byte0=参数 ID，Byte1=`0` 表示成功、`1` 表示非法，Byte2~5 为实际生效值，Byte6 回显事务序号，Byte7=0。当前不实现 Flash 保存，装甲板重启后恢复默认值；其他参数、参数读取、零偏标定仍未实现。为保护采样链路，通信层每次主循环最多发出一帧 CAN 报文。
